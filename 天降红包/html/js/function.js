 ...
 *  speed:速度，默认400ms
 *  time:间隔时间 默认5000
 *  direction:动画方向，vertical=垂直(默认)，horizontal=水平
 */
$.fn.marquee = function(options){
    ...
    this.each(function(){
        //test3
        var _this=$(this);
        var _div=_this.find(".modules");
        var _ul=_this.find("ul");
        var _height=_ul.children("li:first").outerHeight(true);
        var _width=_ul.children("li:first").outerWidth(true);
        var _len=_ul.find("li").size();
        var _butPrev=_this.find(".prev");
        var _butNext=_this.find(".next");
        var trundle;
        var isWheel=false;
        if(options.direction=="vertical"){
            ...
            var _ulLength=_len*_width;
            _ul.width(_ulLength+3);
            _ul.find("li").css({"float":"left"});
            if(_ulLength>_div.width()){
                isWheel=true;
            }
        }
        if(isWheel){
        ...
        function wheelGo(d){
            ...
            _ul.stop().animate({"top":_top,"left":_left},options.speed,function(){
                if(d==1){
                    _ul.append(_ul.find("li:first"));
                    _ul.css({"top":0,"left":0});
                }
                autoWheel();
            });
